...
class Map():
    def __init__(self,type='xyz',filename='../../defFiles/roadDef/defaultRoad.txt',height_ref='WGS'):
    ...
    def loadCSVXYZMap(self,filename):
        ...
        for row in map_reader:
            ...
            if row and not row[0].startswith('#'):
                ...
                # print("map reader reading: "+str(mapline))
                ...
                if len(mapline)>2:
                    ...
                    if firstline==1:
                        ...
                        # print("map reader setting station: "+str(self.S))
    ...
    def station_here(self,latx,lony,altz,type="lla"):
        ...
        twoneighbors_unsort = self.knn_search(gpspos,xymap,2)
        #if the closest point is ahead of the next-closest, flip the order
        if(self.S[twoneighbors_unsort[0]]>self.S[twoneighbors_unsort[1]]):
            twoneighbors = flip(twoneighbors_unsort)
        else:
            twoneighbors = twoneighbors_unsort
    ...
    def relativeMapPoints(self,x=0,y=0,z=0,roll=0,pitch=0,yaw=0,Svector=arange(0,10)):
        ...
        Sdouble = hstack((self.S,self.S+self.S[-1]+sqrt((self.X[0]-self.X[-1])**2+(self.Y[0]-self.Y[-1])**2+(self.Z[0]-self.Z[-1])**2)))
        Xdouble = hstack((self.X,self.X))
        Ydouble = hstack((self.Y,self.Y))
        Zdouble = hstack((self.Z,self.Z))

        Xpts = interp(Squery,Sdouble,Xdouble)
        Ypts = interp(Squery,Sdouble,Ydouble)
        Zpts = interp(Squery,Sdouble,Zdouble)
